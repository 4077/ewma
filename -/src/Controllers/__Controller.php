...
class __Controller
{
    ...
    public function jquery($input = false)
    {
        ...
        // old-todo придумать какой-нибудь символ, который будет только менять базовый путь для селектора, но не загружать файл для этого пути
        // костыль: если в строке есть квадратная скобка, значит двоеточие является частью инстанса
     ...
     * @return string
     */
    public function __selector($input = false)
    {
        $relativeNodePath = false;
        $selector = false;
