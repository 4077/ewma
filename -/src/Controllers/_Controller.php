...
class _Controller
{
    ...
    public function js()
    {
        ...
        if (starts_with($path, ['http://', 'https://'])) {
            ...
            list($path,) = $this->app->controllers->explodeToPathAndInstance($path, $this);
     ...
     */
    public function jquery($input = false, $loadFile = false)
    {
        $relativeNodePath = false;

        list($path, $instance) = $this->app->controllers->explodeToPathAndInstance($input, $this);

        if ($loadFile && false !== strpos($input, ':')) {
            list($relativeNodePath,) = explode(':', $path);
     ...
     * @return \ewma\Js\JqueryBuilder
     */
    public function ___jquery($input = false)
    {
        $relativeNodePath = false;
    ...
    public function _selector($input = false)
    {
        ...
        if (preg_match('/\|(\S*)/', $input, $match)) {
            $instance = $match[1] ?: $this->__meta__->instance;
        }

        list($path, $instance) = $this->app->controllers->explodeToPathAndInstance($input, $this);

        if (false !== strpos($input, ':')) {
            list($relativeNodePath, $selector) = explode(':', $path);
        }
        ...
        $instanceTail = '';
        if ('' !== $instance) {
            $instanceTail = "[instance='" . $instance . "']";
        }

        if (!$selector) {
            $selector = '.' . $nodeId . $instanceTail;
        } else {
            if ($selector == '.' || $selector == '#') {
                $selector .= $nodeId . $instanceTail;
            } else {
                if (!preg_match('/^[a-z0-9_-]$/i', substr($selector, 1, 1))) {
                    $selector = substr($selector, 0, 1) . $nodeId . $instanceTail . substr($selector, 1);
                }
            }
        }
     ...
     * на удаление
     */
    public function ___selector($input = false)
    {
        $output = [];
    ...
    public function widget()
    {
        ...
        $jqueryBuilder = $this->jquery($args[0] ?? false, true);
